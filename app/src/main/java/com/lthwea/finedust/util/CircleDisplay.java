...
public class CircleDisplay extends View {
    ...
    private void init() {
        ...
        mTextPaint.setTextSize(CircleUtils.convertDpToPixel(getResources(), 24f));
    ...
    private void drawText(Canvas c) {
        ...
        String result = TYPE == 1 ? Utils.getPm25ValueStatus(getVALUE()) : Utils.getPm10ValueStatus(getVALUE());


        c.drawText(title, getWidth() / 2, getHeight() / 2 - (mTextPaint.getTextSize()) +10, mTextPaint);

        if( Integer.parseInt(getVALUE()) >= 0){
            if(TYPE != 2){
                c.drawText(mFormatValue.format(mValue * mPhase), getWidth() / 2, getHeight() / 2 + 10, mTextPaint);
            }else{
                c.drawText(SIDO, getWidth() / 2, getHeight() / 2 + 10, mTextPaint);
            }
        }else{
        ...
        c.drawText(result, getWidth() / 2, getHeight() / 2 + (mTextPaint.getTextSize()) + 10, mTextPaint);
    ...
    public void setTextSize(float size) {
        mTextPaint.setTextSize(CircleUtils.convertDpToPixel(getResources(), size));
    }
   ...
   /* @Override
    ...
    public static abstract class CircleUtils {
